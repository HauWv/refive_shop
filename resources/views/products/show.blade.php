        ...
        <div class="skus">
          ...
          <div class="btn-group" data-toggle="buttons">
            @foreach($product->skus as $sku)
              <label class="btn btn-default sku-btn" data-price="{{$sku->price}}" data-stock="{{$sku->stock}}" data-toggle='tooltip' title="{{ $sku->description }}"
    ...
    <div class="product-detail">
      ...
      <div class="tab-content">
        <div role="tabpanel" class="tab-pane active" id="product-detail-tab">
          {!! $product->description !!} <!-- description是用富文本编辑器写的，本身就是HTML代码，不用再转译 -->
        </div>
...
<script>
  $(document).ready(function(){
    ...
    $('.btn-favor').click(function(){
      ...
      .then(function(){   //  请求成功后执行的回调
        swal('操作成功','','success');
      },function(error){  //  请求失败后执行的回调
        //如果返回码是401代表没登录
